...
function ApiSelect<TResponse, TData>({
    ...
    return (
        <div
            className={`relative flex flex-col gap-3 w-full duration-300 select-none`}
            ref={fullContainer}
        >
            <label
            ...
            <div
                onClick={() => handleOpen()}
                className={`transition-all dark:bg-input/30 bg-transparent duration-300 flex px-3 py-2 cursor-pointer justify-between ${styles?.selectClasses ?? "w-full rounded-md border text-primary sm:text-sm"}`}
            >
                <div
